...
class FirebaseService {
  ...
  Future<bool> addTree({
    ...
    debugPrint(
      '[FirebaseService] Position: lat=${position.latitude}, lng=${position.longitude}',
    );
    ...
    try {
      ...
      if (image != null) {
        debugPrint(
          '[FirebaseService] Image provided, starting upload: ${image.name}',
        );
        ...
        try {
          imageUrl = await _uploadImage(image).timeout(
            ...
            onTimeout: () {
              debugPrint(
                '[FirebaseService] ERROR: Image upload timed out after 30 seconds',
              );
              throw FirebaseException(
          ...
          debugPrint(
            '[FirebaseService] Image upload completed in ${uploadDuration.inSeconds}s, URL: $imageUrl',
          );
        } catch (e) {
        ...
        debugPrint(
          '[FirebaseService] No image provided, proceeding without image',
        );
      }
      ...
      debugPrint(
        '[FirebaseService] Payload prepared, adding to Firestore: $payload',
      );
      final firestoreStart = DateTime.now();

      // Check Firestore settings
      debugPrint(
        '[FirebaseService] Firestore settings: persistenceEnabled=${_firestore.settings.persistenceEnabled}',
      );
      ...
      debugPrint(
        '[FirebaseService]   NOTE: If this hangs, check Firebase Console:',
      );
      debugPrint(
        '[FirebaseService]   Authentication > Settings > Authorized domains',
      );
      ...
      try {
        ...
        final testDoc = await collectionRef
            .add({'test': 'hello', 'timestamp': DateTime.now().toString()})
            .timeout(
              const Duration(seconds: 10),
              onTimeout: () {
                debugPrint(
                  '[FirebaseService]   ⏱️ MINIMAL TEST TIMED OUT after 10s',
                );
                debugPrint('[FirebaseService]   ');
                debugPrint(
                  '[FirebaseService]   🚨 CRITICAL: Firestore writes are hanging!',
                );
                debugPrint(
                  '[FirebaseService]   This is almost certainly because:',
                );
                debugPrint(
                  '[FirebaseService]   1. "localhost" is NOT in Firebase Authorized Domains',
                );
                debugPrint(
                  '[FirebaseService]   2. Or WebSocket connections are blocked',
                );
                debugPrint('[FirebaseService]   ');
                debugPrint(
                  '[FirebaseService]   ✅ FIX: Go to Firebase Console:',
                );
                debugPrint(
                  '[FirebaseService]   https://console.firebase.google.com/project/applied-primacy-294221/authentication/settings',
                );
                debugPrint(
                  '[FirebaseService]   Add "localhost" to Authorized domains',
                );
                throw Exception(
                  'Firestore write timed out - check authorized domains',
                );
              },
            );

        debugPrint(
          '[FirebaseService]   ✅ MINIMAL TEST SUCCESS! ID: ${testDoc.id}',
        );
      ...
      debugPrint(
        '[FirebaseService]   Payload size: ${payload.toString().length} chars',
      );
  ...
  DocumentReference? docRef;
      ...
      debugPrint(
        '[FirebaseService] SUCCESS: Tree added to Firestore in ${firestoreDuration.inSeconds}s',
      );
      return true; // Tree added successfully
    } on FirebaseException catch (e) {
      debugPrint(
        '[FirebaseService] ERROR: FirebaseException - plugin: ${e.plugin}, code: ${e.code}, message: ${e.message}',
      );
      throw e; // Re-throw Firebase exceptions
    } catch (e, stackTrace) {
      // Catch any other unexpected errors and wrap them in a FirebaseException
      debugPrint(
        '[FirebaseService] ERROR: Unexpected error while adding tree: $e',
      );
  ...
  Stream<QuerySnapshot> getTrees({int minVerificationScore = -100}) {
    return _firestore
        ...
        .where(
          'verificationScore',
          isGreaterThanOrEqualTo: minVerificationScore,
        )
        .snapshots();
  }
        ...
        try {
          if (data['downvotes'] is List) {
            downvotes = List<String>.from(
              data['downvotes'].whereType<String>(),
            );
          }
        } catch (e) {
  ...
  Future<void> downvoteTree(String treeId, String userId) async {
    ...
    try {
      await _firestore.runTransaction((transaction) async {
        ...
        final List<String> upvotes = List<String>.from(
          snapshot.data()!['upvotes'] ?? [],
        );
        final List<String> downvotes = List<String>.from(
          snapshot.data()!['downvotes'] ?? [],
        );
